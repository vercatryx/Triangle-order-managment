/**
 * Operator inquire current orders only API (new segregated endpoint).
 * POST /api/operator/inquire-orders-current — body: { phone } (or clientId)
 * Returns: { currentOrders } only. 404 when client not found for phone.
...
function logInquireParams(params: Record<string, unknown>) {
  console.log('[operator/inquire-orders-current] POST input payload:', JSON.stringify(params));
}
...
async function resolveAndInquire(clientId: string | undefined, phone: string | undefined) {
  ...
  return {
    data: { currentOrders: result.currentOrders ?? [] },
    status: 200,
  } as const;
}
  ...
  try {
    ...
    if ('error' in outcome) {
      if (outcome.status === 404) {
        console.warn('[operator/inquire-orders-current] 404 — client not found', { clientId, phone, error: outcome.error });
      }
    ...
    console.error('[operator/inquire-orders-current]', err);
