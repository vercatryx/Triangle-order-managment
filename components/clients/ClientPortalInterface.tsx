...
export function ClientPortalInterface({ client: initialClient, statuses, navigators, vendors, menuItems, boxTypes, categories, upcomingOrder, activeOrder, previousOrders, mealCategories, mealItems, foodOrder, mealOrder, boxOrders, clientHistory = [] }: Props) {
                ...
                <div className={stylesClientPortal.scrollableContent}>
                    ...
                    <div style={{ marginTop: '2rem', marginBottom: '1rem' }}>
                        ...
                        >
                            <span style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                ...
                                Recent Orders ({previousOrders?.length || 0})
                            </span>
                            ...
                            <div style={{
                                ...
                                {!previousOrders || previousOrders.length === 0 ? (
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                        {previousOrders.map((order: any, index: number) => (
                                            <div
                                                key={order.id || index}
                                                style={{
                                                    padding: '1rem',
                                            ...
                                            >
                                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem', fontWeight: 600 }}>
                                                    ...
                                                    <span style={{ color: 'var(--text-secondary)', fontSize: '0.8rem' }}>
                                                        {order.scheduled_delivery_date ? new Date(order.scheduled_delivery_date).toLocaleDateString() : 'Date TBD'}
                                                    </span>
                                                </div>
                                                <div style={{ marginBottom: '0.25rem' }}>
                                                    <strong>Service Type:</strong> {order.service_type || 'N/A'}
                                                </div>
                                                {order.total_value !== undefined && (
                                                    <div style={{ marginBottom: '0.25rem' }}>
                                                        <strong>Total Value:</strong> ${parseFloat(order.total_value?.toString() || '0').toFixed(2)}
                                                    </div>
                                                )}
                                                {order.status && (
                                                    <div style={{ marginBottom: '0.25rem' }}>
                                                ...
                                                {order.case_id && (
                                                    <div style={{ marginBottom: '0.25rem', fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
                    ...
                    <div style={{ marginTop: '1rem', marginBottom: '1rem' }}>
                        ...
                        >
                            <span style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                <History size={16} />
                                History ({clientHistory?.length || 0})
                            </span>
                        ...
                        {historyExpanded && (
                            <div style={{
                                ...
                                {!clientHistory || clientHistory.length === 0 ? (
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                        {clientHistory.map((record: any, index: number) => (
                                            <div
                                                key={record.id || index}
                                                style={{
                                                    padding: '1rem',
                                            ...
                                            >
                                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem', fontWeight: 600 }}>
                                                    <span>{record.type || 'Delivery'}</span>
                                                    <span style={{ color: 'var(--text-secondary)', fontSize: '0.8rem' }}>
                                                ...
                                                {record.notes && (
                                                    <div style={{ marginTop: '0.5rem', padding: '0.5rem', backgroundColor: 'var(--bg-surface-hover)', borderRadius: '4px', fontSize: '0.8rem', fontStyle: 'italic' }}>
                                                        {record.notes}
                                                    </div>
                                                )}
                                            </div>
