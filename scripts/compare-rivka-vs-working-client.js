 ...
 * Run with: node scripts/compare-rivka-vs-working-client.js
 */
...
async function getClientOrderData(clientId) {
    ...
    for (const order of upcomingOrders) {
        ...
        if (!allItemsError && allItems) {
            ...
            orderData.orphaned_items = allItems.filter(item => !item.vendor_selection_id);
...
async function main() {
    ...
    if (!rivkaClients || rivkaClients.length === 0) {
        ...
        console.log('\nPlease provide the client ID manually');
        return;
    }
    ...
    if (workingOrder && rivkaOrder) {
        ...
        workingOrder.vendor_selections.forEach((vs, i) => {
        ...
        rivkaOrder.vendor_selections.forEach((vs, i) => {
            console.log(`       VS ${i + 1}: vendor_id=${vs.vendor_id || 'NULL'}, items=${vs.items.length}`);
        });
